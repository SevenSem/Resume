...
class PersonalityCompleted(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        if  not request.user.is_staff:
            ...
            print(x.gender)
            print(x.age)
            personalityDataTest = PersonalityData.objects.all().filter(user=request.user).order_by('-id')[0] 
            ...
            to_predict_data = [x.gender,x.age,avg_o, avg_c, avg_e, avg_a, avg_n]
            personality_data = clf.predict([to_predict_data])
            print(clf.predict([to_predict_data]))

            created = PersonalityData.objects.create(user = request.user,type_o=avg_o*2, gender= x.gender, age=x.age,type_c=avg_c*2, type_e=avg_e*2, type_a=avg_a*2,type_n=avg_n*2,personality=personality_data)
